...
namespace InternshipAPI.Examples
{
    /// <summary>
    ...
    public class AdvancedQueriesExample
    {
        ...
        public async Task<IEnumerable<object>> JoinExamples()
        {
            ...
            var detailedOrderInfo = await _context.Orders
                ...
                {
                    OrderNumber = oui.Order.OrderNumber,
                    CustomerName = oui.User.FullName,
                    ProductName = p.Name,
                    Quantity = oui.OrderItem.Quantity,
                    UnitPrice = oui.OrderItem.UnitPrice,
                    LineTotal = oui.OrderItem.LineTotal
                })
                .ToListAsync();
